...
class YouTubeDownloader:
    ...
    def setting(self):
        ...
        self.im = Image.open(io.BytesIO(self.rawdata)).resize((300, 150), Image.ANTIALIAS)
        self.image= ImageTk.PhotoImage(self.im)
        self.a["image"]=self.image
        self.filesize = self.video.streams.filter(res=self.hd.get(), progressive=self.sound, file_extension='mp4')[
            0].filesize
        self.pathfile = self.chpath + '/' + self.video.title.replace("|", "").replace(":", "") + '.mp4'
        self.spisok_katshestva = []
    ...
    def threads(self, num):
        self.potok = Thread(target=lambda : self.download(num=num), args=())
        self.potok.start()
        self.potoktimer = Thread(target=self.progressbar, args=())
...
if __name__ == "__main__":
    ...
    Label(appwindow, text="Завантажуй з Ютуб безкоштовно", bg="#85C1E9").pack()
