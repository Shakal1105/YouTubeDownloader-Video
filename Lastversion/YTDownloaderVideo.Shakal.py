...
class YouTubeDownloader:
    ...
    def setting(self):
        ...
        self.filesize = self.video.streams.filter(res=self.hd.get(), progressive=self.sound, file_extension='mp4')[
            0].filesize
        self.pathfile = self.chpath + '/' + self.video.title.replace("|", "").replace(":", "").replace("#", "").replace(".","") + '.mp4'
        print(self.pathfile,"\n","C:/Users/Shakal/Desktop/Скульптинг в Blender 28  Краткое руководство для начинающих  Все кисти")
        self.spisok_katshestva = []
